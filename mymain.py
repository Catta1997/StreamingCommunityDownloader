...
def cli_mode():
    ...
    argv = sys.argv[1:]
    try:
        opts, args = getopt.getopt(argv, 'k:yhvosc', ['keyword=', 'crawlpath=', 'downloadpath=', 'onefile'])
...
def main(word):
    ...
    if len(sys.argv) == 1:
        ...
        if tester: keyword = word
        else: keyword = cli_mode()
    ...
    json_data = parsed_html.find('the-search-page')['records-json']
    episodes = json_data
